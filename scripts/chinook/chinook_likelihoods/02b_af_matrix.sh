...
# Define variables.
MAFFOLDER="10b_pop_angsd_n819"
POPNUM=`ls -1 "$MAFFOLDER"/*.mafs.gz | wc -l`
POPS="01_info_files/populations_n819.txt"
TMP="99_temp"
COMMON_SITES="all_pops_common_snps_n819.txt"
OUTPUT="13b_afs_rda_n819"
AFS="common_afs"
...
# Create a txt file of population names.
ls -1 "$MAFFOLDER"/*.mafs.gz | awk -F"/" '{print $2}' | perl -pe 's/\.mafs\.gz//g' > "$POPS"
