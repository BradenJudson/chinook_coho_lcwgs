...
(rast_only <- ggplot() + 
  tidyterra::geom_spatraster(data = terra::crop(f85_bio$bio05, bound), show.legend = T) +
    scale_fill_gradientn(colours = c("#35A052", "#FCFEBB", "red"), 
                         limits = range, oob = scales::squish, name = "bio5",
...
offset_plot <- \(df, variable, cgr_rev, plot_title, abs_cols, brks, rast, LP, rb, rastleg) {
  ...
  if(rb == TRUE) bound <- terra::ext(c(min(df$Longitude)-5, max(df$Longitude)+5,
                                       ...
                                       p1 <- ggplot(data = df) + theme_bw() +
                                         {if(is.null(rast) == FALSE)  tidyterra::geom_spatraster(data = terra::crop(rast, bound), show.legend = F) } +
                                         ...
                                         geom_point(aes(x = Longitude,
                                                    ...
                                                    shape = 21, stroke = 1/2) + 
                                         ggtitle(plot_title) +
                                         coord_sf(xlim = c(max(full_sites$longitude), min(full_sites$longitude)),
                                                  ylim = c(min(full_sites$latitude),  max(full_sites$latitude))) +
...
(offs <- cowplot::plot_grid(plotlist = list(
  ggdraw(offset_plot(chinook, variable = go85, rast = f85_bio$bio05,
                     cgr_rev = FALSE, abs_cols = T, brks = 4,
                     plot_title = NULL,
                     LP = c(0.01, 0.85), rb = FALSE, rastleg = TRUE) +
           ggtitle("Chinook") +
    theme(plot.margin = margin(5,5,5,5, unit = "pt"))) +
    draw_plot(rast_leg, x = 0.90, y = 0.61, height = 0.4, width = 0.4),
  ggdraw(offset_plot(coho, variable = go85, rast = f85_bio$bio05,
                     cgr_rev = FALSE, abs_cols = T, brks = 4, 
                     plot_title = NULL,
                     LP = c(0.01, 0.85), rb = FALSE, rastleg = FALSE) +
           ggtitle("Coho") +
           theme(axis.text.y = element_blank(),
                 axis.ticks.y = element_blank(),
                 plot.margin = margin(5,5,5,0, unit = "pt"))) +
...
(cohoHet <- offset_plot(coho_hets, variable = het, rast = NULL,
                        ...
                        plot_title = NULL, LP = c(0.04, 0.15),
                        rb = TRUE))
...
gc()
ggsave("plots/off/chin_coho_het.tiff", 
       dpi = 300, width = 14, height = 6, bg = 'white')
