...
write.csv(outlier_loadings, "data/rdas/coho_afsGT_4bioclim_3RDA3SD_n650/outlierSNP_loadings_afsGT_n650.csv", row.names = FALSE)
...
cand_df_cor <- as.data.frame(bioCors) %>% 
  ...
  mutate(CHROM = gsub(".2\\_[[:digit:]]*.", ".2", SNP),
         POS   = as.numeric(gsub(".*\\.2_", "", SNP))) %>% 
  rowwise() %>% 
...
(cor_grp <- cand_df_cor %>% group_by(association) %>% tally() %>% mutate(prop = sprintf(n/sum(n)*100, fmt = "%#.2f")))
