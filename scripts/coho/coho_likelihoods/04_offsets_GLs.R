...
pops <- read.table("data/coho_af_rownames_n83.txt", col.names = "pop")

outlier_afs <- read.table("data/rdas/coho_afsGL_4bioclim_3RDA3SD_n650/outlier_afs_matrix_afsGL4bio2PCA_n650.txt", 
                          header = T) %>% `rownames<-`(., pops$pop)

# GF ---------------------------------------------------------------------------

# Read gradient forest model.
gradfor <- readRDS("data/gfs/coho_gForest_19bio2PCA_3SD3RDA_afsGL_n650.RDS")

# Assess predictor importance for the GF model.
png("plots/coho_gradfor_GLafs_importance_n650.png", width = 2000, height = 1200, res = 250)
plot(gradfor)
dev.off()
...
write.csv(genomic_offsets, "data/GOs/coho_offset_19bio3SD_afsGLs_2pca_n650.csv", row.names = F)
write.csv(genomic_offsets, "chin_coho_shiny/data/coho_offset_19bio3SD_afsGLs_2pca_n650.csv", row.names = F)
