...
hets <- read.delim("data/heterozygosity_files/coho_imputed_het_n650.het", sep = "") %>% 
...
write.csv(hets[,c("id","population", "region", "pOHET")], 
          "data/heterozygosity_files/coho_imputed_het_n650.csv", row.names = F)

write.csv(hets %>% group_by(population) %>% summarize(het = mean(pOHET)), 
          "chin_coho_shiny/data/coho_imputed_het_n650.csv", row.names = F)
